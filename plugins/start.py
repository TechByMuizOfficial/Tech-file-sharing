...
async def start_command(client: Client, message: Message):
    ...
    oner_id = ADMINS  # Fetch the owner's ID from config
    ...
    if id == oner_id:
        ...
        if "verify_" in message.text:
            ...
            if IS_VERIFY and not verify_status['is_verified']:
                ...
                await message.reply(f"Your Ads token is expired, refresh your token and try again.\n\nToken Timeout: {get_exp_time(VERIFY_EXPIRE)}\n\nWhat is the token?\n\nThis is an ads token. If you pass 1 ad, you can use the bot for 3 days after passing the ad.", reply_markup=InlineKeyboardMarkup(btn), protect_content=False, quote=True)
